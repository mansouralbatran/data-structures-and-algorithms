...
const checkValues = (obj, value) => {
  let trufale = false;
  Object.keys(obj).forEach((elemnt) => {
    if (obj[elemnt] === value)
      trufale = true;
  });
  return trufale;
...
const updateNumbers = (obj) => {
  let strigarry = [];
  let names = Object.keys(obj).map((elem) => {
    return elem;

  });
  let dates = Object.values(obj).map((elem) => {
    return elem;
  });
  for (let i = 0; i < names.length; i++) {
    strigarry.push(names[i] + ': ' + dates[i]);
  }
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).map((elem)=>{
    if (elem.name===character && elem.children) {
      Child=true;}});
  // Solution code here...
  return Child;
};
