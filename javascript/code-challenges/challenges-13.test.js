...
const longestString = (arr) => { let length = 0; let index;
  if (arr.length > 0) { arr.map((string, idx) => { if (length < string.length) { length = string.length; index = idx; } }); } else { index = -1; }
  return index;};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const firstLetters = (arr) => { let result = []; arr.map(string =>{ result.push(string.charAt(0)); });
  return result;};
...
------------------------------------------------------------------------------------------------ */
const findHappiness = (arr) => { let result =[];
  result = arr.filter(string =>{ if (string.includes(':)')){return string;} });
  return result;};
...
------------------------------------------------------------------------------------------------ */
const standardizePhoneNumbers = (arr) => { let newArr = []; arr.map(item => { let newNumb = item.substring(1, 4) + item.substring(6, 9) + item.substring(10, 14); newArr.push(newNumb); }); return newArr;};
...
const onlyOddChars = (str) => { return ( str.split('').filter((num,idx) =>{ if (idx%2 !==0){return num;} }).join('') ); };
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
