...
const divisibleByFiveTwoToThePower = (input) => {
  let myarr=[];
  input.map(arrnumb=>{let arry2=[];
    arrnumb.map(num=>{

      if (num%5===0 &&typeof(num)==='number'){
        arry2.push(Math.pow(2,num) );
...
let findMaleAndFemale = (data) => {
  ...
  data.map((object,index)=>{
    if (object.gender==='female'||object.gender==='male') {named=named+`${object.name}`;
...
let findShortest = (data) => {
  let short=data[0];
  data.map(ele=>{
    if(Number( ele.height)<Number(short.height)){
      short=ele;
    }
  });
  return short.name;
